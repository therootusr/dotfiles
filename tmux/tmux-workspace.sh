...
function f_attach_existing_window() {
  ...
  while IFS=$'\t' read -r window_session window_name; do
    ...
    existing_session="$window_session"
    break
  done <<< "$windows"

  if [[ -n "$existing_session" ]]; then
    if [[ -n "${TMUX:-}" ]]; then
      f_tmux switch-client -t "$existing_session:$ws_window"
    else
      f_tmux attach-session -t "$existing_session:$ws_window"
    fi
    return 0
  fi
